...
int FSManager::_save_internal(const char* data_id, void* data, uint32_t size, NVSInterface::KeyValueType type){
	...
	DEBUG_TRACE_D(_EXPR_, _MODULE_, "Escribiendo %lu datos en id %s...", (unsigned long)size, data_id);
	switch(type){
		...
		case NVSInterface::TypeString: {
			...
			err = (s != nullptr) ? nvs_set_str(_handle, data_id, s) : ESP_ERR_INVALID_ARG;
			break;
		}
	...
	if(err != ESP_OK){
		DEBUG_TRACE_E(_EXPR_, _MODULE_, "ERR_WR [%s] al escribir key '%s': %s", DEFAULT_NVSInterface_Partition, data_id, esp_err_to_name(err));
		_error = (int)err;
		if(temp_opened){
			_close_internal(false);
		}
	...
	_error = (int)err;
	if(err == ESP_OK){
		DEBUG_TRACE_D(_EXPR_, _MODULE_, "OK: escrito '%s' (%lu bytes) y commit hecho", data_id, (unsigned long)size);
	}else{
		DEBUG_TRACE_E(_EXPR_, _MODULE_, "ERR_COMMIT en key '%s': %s", data_id, esp_err_to_name(err));
	}
	if(temp_opened){
		_close_internal(false);
	}
	...
	switch(type){
		...
		case NVSInterface::TypeBlob: {
			...
			if(blob_size != (size_t)size){
				DEBUG_TRACE_W(_EXPR_, _MODULE_, "WARN_SIZE. Tamaño en NVS (%lu) diferente al buffer (%lu) para id %s", (unsigned long)blob_size, (unsigned long)size, data_id);
